...
async function filterOnlineDomains(domains) {
  ...
  for (let i = 0; i < domains.length; i += concurrency) {
    ...
    if (i + concurrency < domains.length) {
      process.stdout.write(`\r[check] ${online.length} online / ${i + concurrency} checked`);
    }
  }
...
async function generateContent(prompt, systemPrompt) {
  if (!process.env.OLLAMA_OFFLINE) {
    try {
      ...
      console.warn('[ollama] Generation failed, using fallback content:', err.message);
      return null;
    }
  }
  return null;
...
<p>Following these steps, you should now have your project live on a custom is-pro.dev subdomain with free hosting, DNS management, and SSL security.</p>`;
  ...
  const faqs = [
    { q: `Is ${topic.category} setup free on is-pro.dev?`, a: 'Yes, all subdomains on is-pro.dev include free DNS management and SSL certificates.' },
    { q: 'How long does DNS take to propagate?', a: 'Cloudflare typically propagates DNS changes within seconds to a few minutes globally.' },
    { q: 'Can I use this for commercial projects?', a: 'Yes, is-pro.dev subdomains can be used for personal and commercial projects within our fair use policy.' }
  ];
...
<p>The tools available to developers today are remarkably powerful and often free. The key is knowing how to piece them together effectively. Free subdomain providers like is-pro.dev are part of that toolkit.</p>`;
  ...
  const faqs = [
    { q: 'Is is-pro.dev really free to use?', a: 'Yes, is-pro.dev provides free subdomains for developers with no hidden fees.' },
    { q: 'What can I host on an is-pro.dev subdomain?', a: 'Any legitimate project — portfolios, SaaS apps, game servers, APIs, and more.' }
  ];
...
function buildToolWidget(tool) {
  ...
  const row = (c) => `<div style="display:flex;gap:.75rem;flex-wrap:wrap;">${c}</div>`;
  const col = (c) => `<div style="display:flex;flex-direction:column;gap:.75rem;">${c}</div>`;

  const base = (body, msg) => `<div style="background:var(--color-elevated);border:1px solid var(--color-border);border-radius:var(--radius-lg);padding:2rem;margin:1.5rem 0;">${body}${r(msg)}</div>`;
...
async function generateShowcaseIndex(domains) {
  const featured = domains.slice(0, 6).map(d => showcaseCardHTML({
  ...
  const categoryCards = SHOWCASE_TOPICS.map(t => postCardHTML({
  ...
  const headHtml = htmlHead({
    ...
    description: 'Discover amazing projects built by the is-cool-me community. Browse portfolios, apps, tools, and more — all hosted on free is-pro.dev subdomains.',
  ...
  <section class="section" style="padding:var(--space-12) 0;">
    <div class="container">
      ...
      <div class="posts-grid" style="margin-top:2rem;">
        ${domains.length > 0 ? domains.slice(0, 12).map(d => showcaseCardHTML({
          href: `/showcase/${slugify(d.subdomain)}/`,
          title: `${d.subdomain}.is-pro.dev`,
...
async function generateTrendingPage(domains) {
  ...
  const headHtml = htmlHead({
    ...
    description: 'See what is hot in the is-cool-me community. Trending projects, rising stars, and popular developer builds on free subdomains.',
...
async function generateNewPage(domains) {
  const newest = domains.slice(0, 12).map(d => showcaseCardHTML({
    href: `/showcase/${slugify(d.subdomain)}/`,
    title: `${d.subdomain}.is-pro.dev`,
  ...
  const headHtml = htmlHead({
    ...
    description: 'Recently registered subdomains on is-cool-me. Discover the newest projects from the developer community.',
...
async function generateShowcaseProjectPage(domain) {
  ...
  <style>.showcase-grid@media(max-width:640px){.showcase-grid{grid-template-columns:1fr!important}}</style>` + internalLinksHTML(INTERNAL_LINK_SECTIONS) + poweredByFooter(slug);
...
function generateToolsPage() {
  const toolCards = TOOL_PAGES.map(t => postCardHTML({
    href: `/tools/${t.slug}/`,
    title: t.name,
    excerpt: t.description,
    category: t.category,
  ...
  const headHtml = htmlHead({
    ...
    description: 'Free browser-based developer tools: DNS checker, SSL checker, robots.txt generator, sitemap validator, OpenGraph preview, meta tag generator, and more.',
    canonical: `${BASE_URL}/tools/`
  });

  const headerHtml = headerHTML('/tools/');
  const contentHtml = pageHeroHTML('Tools', 'Free Developer Tools', 'Browser-based utilities for DNS, SSL, SEO, and web development. No signup required.') + `
  <section class="section" style="padding:var(--space-12) 0;">
    <div class="container">
      <div class="posts-grid" style="margin-top:2rem;">
        ${toolCards.join('\n')}
...
function generateToolPage(tool) {
  ...
  const headerHtml = headerHTML('/tools/');
  const contentHtml = toolPageHTML({
    name: tool.name,
    description: tool.description,
...
function generateTutorialsPage() {
  const tutorialCards = TUTORIAL_TOPICS.map(t => postCardHTML({
  ...
  const headHtml = htmlHead({
    ...
    description: 'Step-by-step tutorials for deploying projects, configuring DNS, and building on is-pro.dev subdomains.',
    canonical: `${BASE_URL}/tutorials/`
  });

  const headerHtml = headerHTML('/tutorials/');
  const contentHtml = pageHeroHTML('Tutorials', 'Step-by-Step Guides', 'Hands-on tutorials with code examples and real-world deployment scenarios.') + `
  <section class="section" style="padding:var(--space-12) 0;">
    <div class="container">
      <div class="posts-grid" style="margin-top:2rem;">
        ${tutorialCards.join('\n')}
...
function generateComparePage() {
  const compareCards = COMPARE_TOPICS.map(t => postCardHTML({
    href: `/compare/${t.slug}/`,
    title: t.title,
    excerpt: t.summary,
    category: t.category,
  ...
  const headHtml = htmlHead({
    ...
    description: 'Compare free hosting platforms, domain providers, DNS services, and developer tools side by side.',
    canonical: `${BASE_URL}/compare/`
  });

  const headerHtml = headerHTML('/compare/');
  const contentHtml = pageHeroHTML('Compare', 'Platform Comparisons', 'Side-by-side comparisons of hosting platforms, tools, and services for developer projects.') + `
  <section class="section" style="padding:var(--space-12) 0;">
    <div class="container">
      <div class="posts-grid" style="margin-top:2rem;">
        ${compareCards.join('\n')}
...
async function generateSitemap() {
  ...
  const staticPages = [
    '/', '/domains/', '/blog/', '/guides/', '/showcase/', '/trending/',
    '/new/', '/tools/', '/tutorials/', '/compare/', '/about/', '/contact/',
    '/terms/', '/privacy/', '/abuse-report/'
  ];
  urls.push(...staticPages.map(p => ({ loc: BASE_URL + p, priority: p === '/' ? '1.0' : '0.8', changefreq: 'weekly' })));

  urls.push({ loc: BASE_URL + '/guides/', priority: '0.9', changefreq: 'weekly' });
  urls.push({ loc: BASE_URL + '/blog/', priority: '0.9', changefreq: 'weekly' });
  urls.push({ loc: BASE_URL + '/showcase/', priority: '0.9', changefreq: 'daily' });

  const allTopics = [...GUIDE_TOPICS, ...BLOG_TOPICS, ...TUTORIAL_TOPICS, ...COMPARE_TOPICS];
  allTopics.forEach(topic => {
    const section = GUIDE_TOPICS.includes(topic) ? 'guides' :
                    BLOG_TOPICS.includes(topic) ? 'blog' :
                    TUTORIAL_TOPICS.includes(topic) ? 'tutorials' : 'compare';
    urls.push({ loc: BASE_URL + `/${section}/${topic.slug}/`, priority: '0.7', changefreq: 'monthly' });
  });

  TOOL_PAGES.forEach(tool => {
    urls.push({ loc: BASE_URL + `/tools/${tool.slug}/`, priority: '0.6', changefreq: 'monthly' });
  });

  urls.push({ loc: BASE_URL + '/tools/', priority: '0.8', changefreq: 'weekly' });
  urls.push({ loc: BASE_URL + '/tutorials/', priority: '0.8', changefreq: 'weekly' });
  urls.push({ loc: BASE_URL + '/compare/', priority: '0.8', changefreq: 'weekly' });
...
async function main() {
  ...
  const types = args.filter(a => a.startsWith('--type=')).map(a => a.replace('--type=', ''));
  ...
  if (types.length > 0 && !doAll) {
    ...
    console.log('   Available: --type=guides, --type=blog, --type=showcase, --type=tools, --type=tutorials, --type=compare, --type=profiles');
  ...
  if (db.length > 0) {
    ...
    console.log(`📊 ${db.length} domains returning HTTP 200 will be shown in showcase`);
  }

  if (doAll || types.includes('guides') || types.includes('showcase') || types.includes('blog') || types.includes('tools') || types.includes('tutorials') || types.includes('compare') || types.includes('profiles') || types.length === 0) {
  ...
  if (doAll || types.includes('profiles') || types.length === 0) {
    ...
    const usernames = [...new Set(db.map(d => d.owner_username).filter(Boolean))];
  ...
  if (doPush) {
    ...
    try {
      ...
      const status = execSync('git status --porcelain', { shell: '/bin/bash' }).toString().trim();
      if (status) {
        execSync('git commit -m "chore: auto-generate content via AI [skip ci]"', { shell: '/bin/bash' });
