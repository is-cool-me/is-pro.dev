...
(function () {
  ...
  window.copyTextarea = function (btn) {
    var ta = btn.parentElement.parentElement.querySelector('textarea');
    if (!ta) { setError('Nothing to copy.'); return; }
  ...
  window.checkSSL = function () {
    ...
    if (url.indexOf('https://') !== 0) {
      setError('SSL checker requires an HTTPS URL.');
      return;
    }
  ...
  window.generateRobotsTxt = function () {
    ...
    html += '<textarea readonly id="rt-output" style="width:100%;min-height:120px;background:var(--color-bg);border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.75rem;color:var(--color-text);font-family:var(--font-mono);font-size:.8rem;">' + safeContent + '</textarea>';
    html += '<div style="margin-top:.5rem;"><button onclick="copyTextarea(this)" style="padding:.5rem 1rem;background:var(--color-accent);color:#fff;border:none;border-radius:var(--radius-md);cursor:pointer;font-size:.8rem;">Copy to Clipboard</button></div>';
    setResult(html);
  };
  ...
  window.validateSitemap = function () {
    ...
    var html = '<div style="margin-bottom:.5rem;"><strong style="color:var(--color-accent-light)">Sitemap Validation Results</strong></div>';
    ...
    html += '<strong>Issues:</strong> ' + issues.length + '</div>';
  ...
  window.generateMeta = function () {
    ...
    html += '<textarea readonly id="meta-output" style="width:100%;min-height:180px;background:var(--color-bg);border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.75rem;color:var(--color-text);font-family:var(--font-mono);font-size:.8rem;">' + safeContent + '</textarea>';
    html += '<div style="margin-top:.5rem;"><button onclick="copyTextarea(this)" style="padding:.5rem 1rem;background:var(--color-accent);color:#fff;border:none;border-radius:var(--radius-md);cursor:pointer;font-size:.8rem;">Copy to Clipboard</button></div>';
    setResult(html);
  };
  ...
  window.formatJSON = function () {
    ...
    try {
      ...
      html += '<textarea readonly id="json-output" style="width:100%;min-height:150px;background:var(--color-bg);border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.75rem;color:var(--color-text);font-family:var(--font-mono);font-size:.8rem;">' + escapeHtml(formatted) + '</textarea>';
      html += '<div style="margin-top:.5rem;display:flex;gap:.5rem;flex-wrap:wrap;">';
      html += '<button onclick="copyTextarea(this)" style="padding:.5rem 1rem;background:var(--color-accent);color:#fff;border:none;border-radius:var(--radius-md);cursor:pointer;font-size:.8rem;">Copy</button>';
  ...
  window.minifyJSON = function () {
    ...
    try {
      ...
      html += '<div style="margin-top:.5rem;"><button onclick="copyTextarea(this)" style="padding:.5rem 1rem;background:var(--color-accent);color:#fff;border:none;border-radius:var(--radius-md);cursor:pointer;font-size:.8rem;">Copy</button></div>';
      setResult(html);
    } catch (e) {
    ...
    ipPromise.then(function (ip) {
      ...
      fetch('https://ipinfo.io/' + ip + '/json')
        ...
        .catch(function () {
          setResult('<div style="padding:.5rem;border-left:3px solid var(--color-accent);">'
            + '<strong style="color:var(--color-accent-light)">' + escapeHtml(ip) + '</strong><br>'
            + '<span style="color:var(--color-text-muted);font-size:.85rem;">Geolocation data unavailable</span></div>');
        });
    });
  };
  ...
  /* ── runCheck dispatcher (called by generated tool pages) ── */
  window.runCheck = function () {
    var path = window.location.pathname;
    if (path.indexOf('/dns-checker') >= 0) return window.checkDNS();
    if (path.indexOf('/ssl-checker') >= 0) return window.checkSSL();
    if (path.indexOf('/cname-validator') >= 0) return window.checkCNAME();
    if (path.indexOf('/http-headers-checker') >= 0) return window.checkHeaders();
    if (path.indexOf('/ip-lookup') >= 0) return window.lookupIP();
    if (path.indexOf('/subdomain-finder') >= 0) return window.findSubdomains();
    if (path.indexOf('/opengraph-preview') >= 0) return window.checkOG();
    setError('Unknown tool page.');
  };
